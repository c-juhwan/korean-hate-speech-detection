...
class CNNclassifier(tf.keras.Model):
    def __init__(self, **kargs):
        ...
        self.conv1 = tf.keras.layers.Conv1D(filters=kargs['num_filters'], kernel_size=3,
                                            padding='valid', activation=tf.keras.activations.relu,
                                            kernel_constraint=tf.keras.constraints.MaxNorm(max_value=3))
        self.conv2 = tf.keras.layers.Conv1D(filters=kargs['num_filters'], kernel_size=4,
                                            padding='valid', activation=tf.keras.activations.relu,
                                            kernel_constraint=tf.keras.constraints.MaxNorm(max_value=3))
        self.conv3 = tf.keras.layers.Conv1D(filters=kargs['num_filters'], kernel_size=5,
                                            padding='valid', activation=tf.keras.activations.relu,
                                            kernel_constraint=tf.keras.constraints.MaxNorm(max_value=3))                                                                    
        ...
        self.fc1 = tf.keras.layers.Dense(units=kargs['dense_dimension1'], activation=tf.keras.activations.relu)
        self.fc_last = tf.keras.layers.Dense(units=kargs['output_dimension'], activation=tf.keras.activations.sigmoid)
